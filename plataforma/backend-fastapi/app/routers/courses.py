...
@router.post("")
async def create_course(data: CourseCreate, user: User = Depends(require_permission(Permission.COURSE_CREATE)), db: AsyncSession = Depends(get_db)) -> dict:
    service = CourseService(db)
...
async def update_course(course_id: str, data: CourseUpdate, user: User = Depends(require_permission(Permission.COURSE_EDIT)), db: AsyncSession = Depends(get_db)) -> dict:
    service = CourseService(db)
...
async def delete_course(course_id: str, user: User = Depends(require_permission(Permission.COURSE_DELETE)), db: AsyncSession = Depends(get_db)) -> dict:
    service = CourseService(db)

 ...
 * Tests exhaustivos para el servicio de project
 */
...
describe('ProjectService', () => {
  ...
  describe('getAll', () => {
    it('should return all items', async () => {
      ...
      prismaMock.project.findMany.mockResolvedValue(mockData as any);
  ...
  describe('getById', () => {
    it('should return item by id', async () => {
      ...
      prismaMock.project.findUnique.mockResolvedValue(mockItem as any);
  ...
  describe('create', () => {
    it('should create new item', async () => {
      ...
      prismaMock.project.create.mockResolvedValue(created as any);
  ...
  describe('update', () => {
    it('should update existing item', async () => {
      ...
      prismaMock.project.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.project.update.mockResolvedValue(updated as any);
    ...
    it('should handle partial updates', async () => {
      ...
      prismaMock.project.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.project.update.mockResolvedValue({ id: 'test-id', ...partialUpdate } as any);
  ...
  describe('delete', () => {
    it('should delete item', async () => {
      prismaMock.project.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.project.delete.mockResolvedValue({ id: 'test-id' } as any);
  ...
  describe('caching', () => {
    ...
    it('should invalidate cache on update', async () => {
      ...
      prismaMock.project.findUnique.mockResolvedValue(mockItem as any);
      prismaMock.project.update.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);

      await service.getById('test-id');
      await service.update('test-id', { name: 'Updated' });

      // Cache should be invalidated after update
      prismaMock.project.findUnique.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);
      await service.getById('test-id');

      expect(prismaMock.project.findUnique).toHaveBeenCalledTimes(3);
    });
  });
  ...
  describe('error handling', () => {
    it('should handle network errors', async () => {
      prismaMock.project.findMany.mockRejectedValue(new Error('ECONNREFUSED'));

      await expect(service.getAll()).rejects.toThrow('Database connection failed');
    });

    it('should handle timeout errors', async () => {
      prismaMock.project.findMany.mockRejectedValue(new Error('Query timeout'));

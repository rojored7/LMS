/**
 * email Service Tests
 * Tests exhaustivos para el servicio de email
 */
...
describe('EmailService', () => {
  ...
  describe('getAll', () => {
    it('should return all items', async () => {
      ...
      prismaMock.email.findMany.mockResolvedValue(mockData as any);

      const result = await service.getAll();

      expect(result).toEqual(mockData);
      expect(prismaMock.email.findMany).toHaveBeenCalledTimes(1);
    });
    ...
    it('should handle database errors', async () => {
      prismaMock.email.findMany.mockRejectedValue(new Error('Database error'));
  ...
  describe('getById', () => {
    it('should return item by id', async () => {
      ...
      prismaMock.email.findUnique.mockResolvedValue(mockItem as any);

      const result = await service.getById('test-id');

      expect(result).toEqual(mockItem);
      expect(prismaMock.email.findUnique).toHaveBeenCalledWith({
        where: { id: 'test-id' }
      });
    });
  ...
  describe('create', () => {
    it('should create new item', async () => {
      ...
      prismaMock.email.create.mockResolvedValue(created as any);
  ...
  describe('update', () => {
    it('should update existing item', async () => {
      ...
      prismaMock.email.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.email.update.mockResolvedValue(updated as any);
    ...
    it('should handle partial updates', async () => {
      ...
      prismaMock.email.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.email.update.mockResolvedValue({ id: 'test-id', ...partialUpdate } as any);
  ...
  describe('delete', () => {
    it('should delete item', async () => {
      prismaMock.email.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.email.delete.mockResolvedValue({ id: 'test-id' } as any);

      await service.delete('test-id');

      expect(prismaMock.email.delete).toHaveBeenCalledWith({
        where: { id: 'test-id' }
      });
    });
    ...
    it('should handle deletion with dependencies', async () => {
      prismaMock.email.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.email.delete.mockRejectedValue({
        code: 'P2003',
  ...
  describe('caching', () => {
    it('should cache frequently accessed items', async () => {
      ...
      expect(prismaMock.email.findUnique).toHaveBeenCalledTimes(1);
    });
    ...
    it('should invalidate cache on update', async () => {
      ...
      prismaMock.email.findUnique.mockResolvedValue(mockItem as any);
      prismaMock.email.update.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);

      await service.getById('test-id');
      await service.update('test-id', { name: 'Updated' });

      // Cache should be invalidated after update
      prismaMock.email.findUnique.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);
      await service.getById('test-id');

      expect(prismaMock.email.findUnique).toHaveBeenCalledTimes(3);
    });
  });
  ...
  describe('error handling', () => {
    it('should handle network errors', async () => {
      prismaMock.email.findMany.mockRejectedValue(new Error('ECONNREFUSED'));

      await expect(service.getAll()).rejects.toThrow('Database connection failed');
    });

    it('should handle timeout errors', async () => {
      prismaMock.email.findMany.mockRejectedValue(new Error('Query timeout'));

      await expect(service.getAll()).rejects.toThrow('Query timeout');
    });

    it('should retry on transient errors', async () => {
      prismaMock.email.findUnique
      ...
      expect(prismaMock.email.findUnique).toHaveBeenCalledTimes(2);
    });
  });
});

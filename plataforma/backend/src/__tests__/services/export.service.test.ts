 ...
 * Tests exhaustivos para el servicio de export
 */
...
describe('ExportService', () => {
  ...
  describe('getAll', () => {
    it('should return all items', async () => {
      ...
      prismaMock.export.findMany.mockResolvedValue(mockData as any);

      const result = await service.getAll();

      expect(result).toEqual(mockData);
      expect(prismaMock.export.findMany).toHaveBeenCalledTimes(1);
    });
    ...
    it('should handle database errors', async () => {
      prismaMock.export.findMany.mockRejectedValue(new Error('Database error'));
  ...
  describe('getById', () => {
    it('should return item by id', async () => {
      ...
      prismaMock.export.findUnique.mockResolvedValue(mockItem as any);

      const result = await service.getById('test-id');

      expect(result).toEqual(mockItem);
      expect(prismaMock.export.findUnique).toHaveBeenCalledWith({
        where: { id: 'test-id' }
      });
    });

    it('should throw error if item not found', async () => {
      prismaMock.export.findUnique.mockResolvedValue(null);
  ...
  describe('create', () => {
    it('should create new item', async () => {
      ...
      prismaMock.export.create.mockResolvedValue(created as any);

      const result = await service.create(newData);

      expect(result).toEqual(created);
      expect(prismaMock.export.create).toHaveBeenCalledWith({
        data: newData
      });
    });
    ...
    it('should handle unique constraint violations', async () => {
      ...
      prismaMock.export.create.mockRejectedValue({
        code: 'P2002',
        meta: { target: ['name'] }
      });
  ...
  describe('update', () => {
    it('should update existing item', async () => {
      ...
      prismaMock.export.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.export.update.mockResolvedValue(updated as any);

      const result = await service.update('test-id', updateData);

      expect(result).toEqual(updated);
      expect(prismaMock.export.update).toHaveBeenCalledWith({
        where: { id: 'test-id' },
        data: updateData
      });
    });
    ...
    it('should handle partial updates', async () => {
      ...
      prismaMock.export.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.export.update.mockResolvedValue({ id: 'test-id', ...partialUpdate } as any);
  ...
  describe('delete', () => {
    it('should delete item', async () => {
      prismaMock.export.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.export.delete.mockResolvedValue({ id: 'test-id' } as any);

      await service.delete('test-id');

      expect(prismaMock.export.delete).toHaveBeenCalledWith({
        where: { id: 'test-id' }
      });
    });
    ...
    it('should handle deletion with dependencies', async () => {
      prismaMock.export.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.export.delete.mockRejectedValue({
        code: 'P2003',
    ...
    it('should soft delete when configured', async () => {
      prismaMock.export.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.export.update.mockResolvedValue({
        id: 'test-id',
        deletedAt: new Date()
      } as any);
  ...
  describe('caching', () => {
    it('should cache frequently accessed items', async () => {
      ...
      expect(prismaMock.export.findUnique).toHaveBeenCalledTimes(1);
    });
    ...
    it('should invalidate cache on update', async () => {
      ...
      prismaMock.export.findUnique.mockResolvedValue(mockItem as any);
      prismaMock.export.update.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);

      await service.getById('test-id');
      await service.update('test-id', { name: 'Updated' });

      // Cache should be invalidated after update
      prismaMock.export.findUnique.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);
      await service.getById('test-id');

      expect(prismaMock.export.findUnique).toHaveBeenCalledTimes(3);
    });
  });
  ...
  describe('error handling', () => {
    it('should handle network errors', async () => {
      prismaMock.export.findMany.mockRejectedValue(new Error('ECONNREFUSED'));

      await expect(service.getAll()).rejects.toThrow('Database connection failed');
    });

    it('should handle timeout errors', async () => {
      prismaMock.export.findMany.mockRejectedValue(new Error('Query timeout'));

      await expect(service.getAll()).rejects.toThrow('Query timeout');
    });

    it('should retry on transient errors', async () => {
      prismaMock.export.findUnique
      ...
      expect(prismaMock.export.findUnique).toHaveBeenCalledTimes(2);
    });
  });
});

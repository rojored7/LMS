 ...
 * Tests exhaustivos para el servicio de certificate
 */
...
describe('CertificateService', () => {
  ...
  describe('getAll', () => {
    it('should return all items', async () => {
      ...
      prismaMock.certificate.findMany.mockResolvedValue(mockData as any);

      const result = await service.getAll();

      expect(result).toEqual(mockData);
      expect(prismaMock.certificate.findMany).toHaveBeenCalledTimes(1);
    });
    ...
    it('should handle database errors', async () => {
      prismaMock.certificate.findMany.mockRejectedValue(new Error('Database error'));
  ...
  describe('getById', () => {
    it('should return item by id', async () => {
      ...
      prismaMock.certificate.findUnique.mockResolvedValue(mockItem as any);
  ...
  describe('create', () => {
    it('should create new item', async () => {
      ...
      prismaMock.certificate.create.mockResolvedValue(created as any);
    ...
    it('should handle unique constraint violations', async () => {
      ...
      prismaMock.certificate.create.mockRejectedValue({
        code: 'P2002',
        meta: { target: ['name'] }
      });
  ...
  describe('update', () => {
    it('should update existing item', async () => {
      ...
      prismaMock.certificate.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.certificate.update.mockResolvedValue(updated as any);
    ...
    it('should handle partial updates', async () => {
      ...
      prismaMock.certificate.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.certificate.update.mockResolvedValue({ id: 'test-id', ...partialUpdate } as any);
  ...
  describe('delete', () => {
    it('should delete item', async () => {
      prismaMock.certificate.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.certificate.delete.mockResolvedValue({ id: 'test-id' } as any);

      await service.delete('test-id');

      expect(prismaMock.certificate.delete).toHaveBeenCalledWith({
        where: { id: 'test-id' }
      });
    });
    ...
    it('should handle deletion with dependencies', async () => {
      prismaMock.certificate.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.certificate.delete.mockRejectedValue({
        code: 'P2003',
  ...
  describe('caching', () => {
    it('should cache frequently accessed items', async () => {
      ...
      expect(prismaMock.certificate.findUnique).toHaveBeenCalledTimes(1);
    });
    ...
    it('should invalidate cache on update', async () => {
      ...
      prismaMock.certificate.findUnique.mockResolvedValue(mockItem as any);
      prismaMock.certificate.update.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);

      await service.getById('test-id');
      await service.update('test-id', { name: 'Updated' });

      // Cache should be invalidated after update
      prismaMock.certificate.findUnique.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);
      await service.getById('test-id');

      expect(prismaMock.certificate.findUnique).toHaveBeenCalledTimes(3);
    });
  });
  ...
  describe('error handling', () => {
    it('should handle network errors', async () => {
      prismaMock.certificate.findMany.mockRejectedValue(new Error('ECONNREFUSED'));

      await expect(service.getAll()).rejects.toThrow('Database connection failed');
    });

    it('should handle timeout errors', async () => {
      prismaMock.certificate.findMany.mockRejectedValue(new Error('Query timeout'));
    ...
    it('should retry on transient errors', async () => {
      ...
      expect(prismaMock.certificate.findUnique).toHaveBeenCalledTimes(2);
    });
  });
});

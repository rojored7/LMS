/**
 * badge Service Tests
 * Tests exhaustivos para el servicio de badge
 */
...
describe('BadgeService', () => {
  ...
  describe('getAll', () => {
    it('should return all items', async () => {
      ...
      prismaMock.badge.findMany.mockResolvedValue(mockData as any);

      const result = await service.getAll();

      expect(result).toEqual(mockData);
      expect(prismaMock.badge.findMany).toHaveBeenCalledTimes(1);
    });
    ...
    it('should handle database errors', async () => {
      prismaMock.badge.findMany.mockRejectedValue(new Error('Database error'));
  ...
  describe('getById', () => {
    it('should return item by id', async () => {
      ...
      prismaMock.badge.findUnique.mockResolvedValue(mockItem as any);

      const result = await service.getById('test-id');

      expect(result).toEqual(mockItem);
      expect(prismaMock.badge.findUnique).toHaveBeenCalledWith({
        where: { id: 'test-id' }
      });
    });
  ...
  describe('create', () => {
    it('should create new item', async () => {
      ...
      prismaMock.badge.create.mockResolvedValue(created as any);
  ...
  describe('update', () => {
    it('should update existing item', async () => {
      ...
      prismaMock.badge.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.badge.update.mockResolvedValue(updated as any);

      const result = await service.update('test-id', updateData);

      expect(result).toEqual(updated);
      expect(prismaMock.badge.update).toHaveBeenCalledWith({
        where: { id: 'test-id' },
        data: updateData
      });
    });
    ...
    it('should handle partial updates', async () => {
      ...
      prismaMock.badge.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.badge.update.mockResolvedValue({ id: 'test-id', ...partialUpdate } as any);
  ...
  describe('delete', () => {
    it('should delete item', async () => {
      prismaMock.badge.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.badge.delete.mockResolvedValue({ id: 'test-id' } as any);

      await service.delete('test-id');

      expect(prismaMock.badge.delete).toHaveBeenCalledWith({
        where: { id: 'test-id' }
      });
    });
    ...
    it('should handle deletion with dependencies', async () => {
      prismaMock.badge.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.badge.delete.mockRejectedValue({
        code: 'P2003',
    ...
    it('should soft delete when configured', async () => {
      prismaMock.badge.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.badge.update.mockResolvedValue({
        id: 'test-id',
        deletedAt: new Date()
      } as any);
  ...
  describe('caching', () => {
    it('should cache frequently accessed items', async () => {
      ...
      expect(prismaMock.badge.findUnique).toHaveBeenCalledTimes(1);
    });
    ...
    it('should invalidate cache on update', async () => {
      ...
      prismaMock.badge.findUnique.mockResolvedValue(mockItem as any);
      prismaMock.badge.update.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);

      await service.getById('test-id');
      await service.update('test-id', { name: 'Updated' });

      // Cache should be invalidated after update
      prismaMock.badge.findUnique.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);
      await service.getById('test-id');

      expect(prismaMock.badge.findUnique).toHaveBeenCalledTimes(3);
    });
  });
  ...
  describe('error handling', () => {
    it('should handle network errors', async () => {
      prismaMock.badge.findMany.mockRejectedValue(new Error('ECONNREFUSED'));

      await expect(service.getAll()).rejects.toThrow('Database connection failed');
    });

    it('should handle timeout errors', async () => {
      prismaMock.badge.findMany.mockRejectedValue(new Error('Query timeout'));

      await expect(service.getAll()).rejects.toThrow('Query timeout');
    });

    it('should retry on transient errors', async () => {
      prismaMock.badge.findUnique
      ...
      expect(prismaMock.badge.findUnique).toHaveBeenCalledTimes(2);
    });
  });
});

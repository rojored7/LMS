/**
 * lab Service Tests
 * Tests exhaustivos para el servicio de lab
 */
...
describe('LabService', () => {
  ...
  describe('getAll', () => {
    it('should return all items', async () => {
      ...
      prismaMock.lab.findMany.mockResolvedValue(mockData as any);

      const result = await service.getAll();

      expect(result).toEqual(mockData);
      expect(prismaMock.lab.findMany).toHaveBeenCalledTimes(1);
    });
    ...
    it('should filter by query parameters', async () => {
      ...
      expect(prismaMock.lab.findMany).toHaveBeenCalledWith(
    ...
    it('should handle database errors', async () => {
      prismaMock.lab.findMany.mockRejectedValue(new Error('Database error'));
  ...
  describe('getById', () => {
    it('should return item by id', async () => {
      ...
      prismaMock.lab.findUnique.mockResolvedValue(mockItem as any);

      const result = await service.getById('test-id');

      expect(result).toEqual(mockItem);
      expect(prismaMock.lab.findUnique).toHaveBeenCalledWith({
        where: { id: 'test-id' }
      });
    });
  ...
  describe('create', () => {
    it('should create new item', async () => {
      ...
      prismaMock.lab.create.mockResolvedValue(created as any);

      const result = await service.create(newData);

      expect(result).toEqual(created);
      expect(prismaMock.lab.create).toHaveBeenCalledWith({
        data: newData
      });
    });
    ...
    it('should handle unique constraint violations', async () => {
      ...
      prismaMock.lab.create.mockRejectedValue({
        code: 'P2002',
        meta: { target: ['name'] }
      });
  ...
  describe('update', () => {
    it('should update existing item', async () => {
      ...
      prismaMock.lab.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.lab.update.mockResolvedValue(updated as any);

      const result = await service.update('test-id', updateData);

      expect(result).toEqual(updated);
      expect(prismaMock.lab.update).toHaveBeenCalledWith({
        where: { id: 'test-id' },
        data: updateData
      });
    });
    ...
    it('should handle partial updates', async () => {
      ...
      prismaMock.lab.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.lab.update.mockResolvedValue({ id: 'test-id', ...partialUpdate } as any);
  ...
  describe('delete', () => {
    it('should delete item', async () => {
      prismaMock.lab.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.lab.delete.mockResolvedValue({ id: 'test-id' } as any);

      await service.delete('test-id');

      expect(prismaMock.lab.delete).toHaveBeenCalledWith({
        where: { id: 'test-id' }
      });
    });
    ...
    it('should handle deletion with dependencies', async () => {
      prismaMock.lab.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.lab.delete.mockRejectedValue({
        code: 'P2003',
    ...
    it('should soft delete when configured', async () => {
      prismaMock.lab.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.lab.update.mockResolvedValue({
        id: 'test-id',
        deletedAt: new Date()
      } as any);
  ...
  describe('caching', () => {
    it('should cache frequently accessed items', async () => {
      ...
      expect(prismaMock.lab.findUnique).toHaveBeenCalledTimes(1);
    });
    ...
    it('should invalidate cache on update', async () => {
      ...
      prismaMock.lab.findUnique.mockResolvedValue(mockItem as any);
      prismaMock.lab.update.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);

      await service.getById('test-id');
      await service.update('test-id', { name: 'Updated' });

      // Cache should be invalidated after update
      prismaMock.lab.findUnique.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);
      await service.getById('test-id');

      expect(prismaMock.lab.findUnique).toHaveBeenCalledTimes(3);
    });
  });
  ...
  describe('error handling', () => {
    it('should handle network errors', async () => {
      prismaMock.lab.findMany.mockRejectedValue(new Error('ECONNREFUSED'));

      await expect(service.getAll()).rejects.toThrow('Database connection failed');
    });

    it('should handle timeout errors', async () => {
      prismaMock.lab.findMany.mockRejectedValue(new Error('Query timeout'));

      await expect(service.getAll()).rejects.toThrow('Query timeout');
    });

    it('should retry on transient errors', async () => {
      prismaMock.lab.findUnique
      ...
      expect(prismaMock.lab.findUnique).toHaveBeenCalledTimes(2);
    });
  });
});

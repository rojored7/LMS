 ...
 * Tests exhaustivos para el servicio de storage
 */
...
describe('StorageService', () => {
  ...
  describe('getAll', () => {
    it('should return all items', async () => {
      ...
      prismaMock.storage.findMany.mockResolvedValue(mockData as any);

      const result = await service.getAll();

      expect(result).toEqual(mockData);
      expect(prismaMock.storage.findMany).toHaveBeenCalledTimes(1);
    });
    ...
    it('should filter by query parameters', async () => {
      ...
      expect(prismaMock.storage.findMany).toHaveBeenCalledWith(
    ...
    it('should handle database errors', async () => {
      prismaMock.storage.findMany.mockRejectedValue(new Error('Database error'));
  ...
  describe('getById', () => {
    it('should return item by id', async () => {
      ...
      prismaMock.storage.findUnique.mockResolvedValue(mockItem as any);
    ...
    it('should throw error if item not found', async () => {
      prismaMock.storage.findUnique.mockResolvedValue(null);
  ...
  describe('create', () => {
    it('should create new item', async () => {
      ...
      prismaMock.storage.create.mockResolvedValue(created as any);

      const result = await service.create(newData);

      expect(result).toEqual(created);
      expect(prismaMock.storage.create).toHaveBeenCalledWith({
        data: newData
      });
    });
    ...
    it('should handle unique constraint violations', async () => {
      ...
      prismaMock.storage.create.mockRejectedValue({
        code: 'P2002',
        meta: { target: ['name'] }
      });
  ...
  describe('update', () => {
    it('should update existing item', async () => {
      ...
      prismaMock.storage.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.storage.update.mockResolvedValue(updated as any);

      const result = await service.update('test-id', updateData);

      expect(result).toEqual(updated);
      expect(prismaMock.storage.update).toHaveBeenCalledWith({
        where: { id: 'test-id' },
        data: updateData
      });
    });
    ...
    it('should handle partial updates', async () => {
      ...
      prismaMock.storage.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.storage.update.mockResolvedValue({ id: 'test-id', ...partialUpdate } as any);
  ...
  describe('delete', () => {
    it('should delete item', async () => {
      prismaMock.storage.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.storage.delete.mockResolvedValue({ id: 'test-id' } as any);
    ...
    it('should handle deletion with dependencies', async () => {
      prismaMock.storage.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.storage.delete.mockRejectedValue({
        code: 'P2003',
    ...
    it('should soft delete when configured', async () => {
      prismaMock.storage.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.storage.update.mockResolvedValue({
        id: 'test-id',
        deletedAt: new Date()
      } as any);
  ...
  describe('caching', () => {
    it('should cache frequently accessed items', async () => {
      ...
      expect(prismaMock.storage.findUnique).toHaveBeenCalledTimes(1);
    });
    ...
    it('should invalidate cache on update', async () => {
      ...
      prismaMock.storage.findUnique.mockResolvedValue(mockItem as any);
      prismaMock.storage.update.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);

      await service.getById('test-id');
      await service.update('test-id', { name: 'Updated' });

      // Cache should be invalidated after update
      prismaMock.storage.findUnique.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);
      await service.getById('test-id');

      expect(prismaMock.storage.findUnique).toHaveBeenCalledTimes(3);
    });
  });
  ...
  describe('error handling', () => {
    it('should handle network errors', async () => {
      prismaMock.storage.findMany.mockRejectedValue(new Error('ECONNREFUSED'));

      await expect(service.getAll()).rejects.toThrow('Database connection failed');
    });

    it('should handle timeout errors', async () => {
      prismaMock.storage.findMany.mockRejectedValue(new Error('Query timeout'));

      await expect(service.getAll()).rejects.toThrow('Query timeout');
    });

    it('should retry on transient errors', async () => {
      prismaMock.storage.findUnique
      ...
      expect(prismaMock.storage.findUnique).toHaveBeenCalledTimes(2);
    });
  });
});

 ...
 * Tests exhaustivos para el servicio de progress
 */
...
describe('ProgressService', () => {
  ...
  describe('getAll', () => {
    it('should return all items', async () => {
      ...
      prismaMock.progress.findMany.mockResolvedValue(mockData as any);

      const result = await service.getAll();

      expect(result).toEqual(mockData);
      expect(prismaMock.progress.findMany).toHaveBeenCalledTimes(1);
    });
    ...
    it('should filter by query parameters', async () => {
      ...
      expect(prismaMock.progress.findMany).toHaveBeenCalledWith(
    ...
    it('should handle database errors', async () => {
      prismaMock.progress.findMany.mockRejectedValue(new Error('Database error'));
  ...
  describe('getById', () => {
    it('should return item by id', async () => {
      ...
      prismaMock.progress.findUnique.mockResolvedValue(mockItem as any);

      const result = await service.getById('test-id');

      expect(result).toEqual(mockItem);
      expect(prismaMock.progress.findUnique).toHaveBeenCalledWith({
        where: { id: 'test-id' }
      });
    });
  ...
  describe('create', () => {
    it('should create new item', async () => {
      ...
      prismaMock.progress.create.mockResolvedValue(created as any);

      const result = await service.create(newData);

      expect(result).toEqual(created);
      expect(prismaMock.progress.create).toHaveBeenCalledWith({
        data: newData
      });
    });
    ...
    it('should handle unique constraint violations', async () => {
      ...
      prismaMock.progress.create.mockRejectedValue({
        code: 'P2002',
        meta: { target: ['name'] }
      });
  ...
  describe('update', () => {
    it('should update existing item', async () => {
      ...
      prismaMock.progress.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.progress.update.mockResolvedValue(updated as any);
    ...
    it('should handle partial updates', async () => {
      ...
      prismaMock.progress.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.progress.update.mockResolvedValue({ id: 'test-id', ...partialUpdate } as any);
  ...
  describe('delete', () => {
    it('should delete item', async () => {
      prismaMock.progress.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.progress.delete.mockResolvedValue({ id: 'test-id' } as any);

      await service.delete('test-id');

      expect(prismaMock.progress.delete).toHaveBeenCalledWith({
        where: { id: 'test-id' }
      });
    });
    ...
    it('should handle deletion with dependencies', async () => {
      prismaMock.progress.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.progress.delete.mockRejectedValue({
        code: 'P2003',
  ...
  describe('caching', () => {
    it('should cache frequently accessed items', async () => {
      ...
      expect(prismaMock.progress.findUnique).toHaveBeenCalledTimes(1);
    });
    ...
    it('should invalidate cache on update', async () => {
      ...
      prismaMock.progress.findUnique.mockResolvedValue(mockItem as any);
      prismaMock.progress.update.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);

      await service.getById('test-id');
      await service.update('test-id', { name: 'Updated' });

      // Cache should be invalidated after update
      prismaMock.progress.findUnique.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);
      await service.getById('test-id');

      expect(prismaMock.progress.findUnique).toHaveBeenCalledTimes(3);
    });
  });
  ...
  describe('error handling', () => {
    it('should handle network errors', async () => {
      prismaMock.progress.findMany.mockRejectedValue(new Error('ECONNREFUSED'));

      await expect(service.getAll()).rejects.toThrow('Database connection failed');
    });

    it('should handle timeout errors', async () => {
      prismaMock.progress.findMany.mockRejectedValue(new Error('Query timeout'));
    ...
    it('should retry on transient errors', async () => {
      ...
      expect(prismaMock.progress.findUnique).toHaveBeenCalledTimes(2);
    });
  });
});

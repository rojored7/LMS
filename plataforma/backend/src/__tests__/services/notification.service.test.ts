 ...
 * Tests exhaustivos para el servicio de notification
 */
...
describe('NotificationService', () => {
  ...
  describe('getById', () => {
    it('should return item by id', async () => {
      ...
      prismaMock.notification.findUnique.mockResolvedValue(mockItem as any);
  ...
  describe('update', () => {
    it('should update existing item', async () => {
      ...
      prismaMock.notification.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.notification.update.mockResolvedValue(updated as any);
    ...
    it('should handle partial updates', async () => {
      ...
      prismaMock.notification.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.notification.update.mockResolvedValue({ id: 'test-id', ...partialUpdate } as any);
  ...
  describe('delete', () => {
    it('should delete item', async () => {
      prismaMock.notification.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.notification.delete.mockResolvedValue({ id: 'test-id' } as any);
    ...
    it('should handle deletion with dependencies', async () => {
      prismaMock.notification.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.notification.delete.mockRejectedValue({
        code: 'P2003',
  ...
  describe('caching', () => {
    ...
    it('should invalidate cache on update', async () => {
      ...
      prismaMock.notification.findUnique.mockResolvedValue(mockItem as any);
      prismaMock.notification.update.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);

      await service.getById('test-id');
      await service.update('test-id', { name: 'Updated' });

      // Cache should be invalidated after update
      prismaMock.notification.findUnique.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);
      await service.getById('test-id');
  ...
  describe('error handling', () => {
    it('should handle network errors', async () => {
      prismaMock.notification.findMany.mockRejectedValue(new Error('ECONNREFUSED'));

      await expect(service.getAll()).rejects.toThrow('Database connection failed');
    });

    it('should handle timeout errors', async () => {
      prismaMock.notification.findMany.mockRejectedValue(new Error('Query timeout'));

/**
 * trainingProfile Service Tests
 * Tests exhaustivos para el servicio de trainingprofile
 */
...
describe('TrainingProfileService', () => {
  ...
  beforeEach(() => {
    service = new TrainingProfileService();
    resetPrismaMocks();
  });
  ...
  describe('getAll', () => {
    it('should return all items', async () => {
      ...
      prismaMock.trainingProfile.findMany.mockResolvedValue(mockData as any);

      const result = await service.getAll();

      expect(result).toEqual(mockData);
      expect(prismaMock.trainingProfile.findMany).toHaveBeenCalledTimes(1);
    });
    ...
    it('should filter by query parameters', async () => {
      ...
      expect(prismaMock.trainingProfile.findMany).toHaveBeenCalledWith(
    ...
    it('should handle database errors', async () => {
      prismaMock.trainingProfile.findMany.mockRejectedValue(new Error('Database error'));
  ...
  describe('getById', () => {
    it('should return item by id', async () => {
      ...
      prismaMock.trainingProfile.findUnique.mockResolvedValue(mockItem as any);

      const result = await service.getById('test-id');

      expect(result).toEqual(mockItem);
      expect(prismaMock.trainingProfile.findUnique).toHaveBeenCalledWith({
        where: { id: 'test-id' }
      });
    });
  ...
  describe('create', () => {
    it('should create new item', async () => {
      ...
      prismaMock.trainingProfile.create.mockResolvedValue(created as any);

      const result = await service.create(newData);

      expect(result).toEqual(created);
      expect(prismaMock.trainingProfile.create).toHaveBeenCalledWith({
        data: newData
      });
    });
    ...
    it('should handle unique constraint violations', async () => {
      ...
      prismaMock.trainingProfile.create.mockRejectedValue({
        code: 'P2002',
        meta: { target: ['name'] }
      });
  ...
  describe('update', () => {
    it('should update existing item', async () => {
      ...
      prismaMock.trainingProfile.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.trainingProfile.update.mockResolvedValue(updated as any);

      const result = await service.update('test-id', updateData);

      expect(result).toEqual(updated);
      expect(prismaMock.trainingProfile.update).toHaveBeenCalledWith({
        where: { id: 'test-id' },
        data: updateData
      });
    });
    ...
    it('should handle partial updates', async () => {
      ...
      prismaMock.trainingProfile.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.trainingProfile.update.mockResolvedValue({ id: 'test-id', ...partialUpdate } as any);
  ...
  describe('delete', () => {
    it('should delete item', async () => {
      prismaMock.trainingProfile.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.trainingProfile.delete.mockResolvedValue({ id: 'test-id' } as any);

      await service.delete('test-id');

      expect(prismaMock.trainingProfile.delete).toHaveBeenCalledWith({
        where: { id: 'test-id' }
      });
    });
    ...
    it('should handle deletion with dependencies', async () => {
      prismaMock.trainingProfile.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.trainingProfile.delete.mockRejectedValue({
        code: 'P2003',
    ...
    it('should soft delete when configured', async () => {
      prismaMock.trainingProfile.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.trainingProfile.update.mockResolvedValue({
        id: 'test-id',
        deletedAt: new Date()
      } as any);
  ...
  describe('caching', () => {
    it('should cache frequently accessed items', async () => {
      ...
      expect(prismaMock.trainingProfile.findUnique).toHaveBeenCalledTimes(1);
    });
    ...
    it('should invalidate cache on update', async () => {
      ...
      prismaMock.trainingProfile.findUnique.mockResolvedValue(mockItem as any);
      prismaMock.trainingProfile.update.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);

      await service.getById('test-id');
      await service.update('test-id', { name: 'Updated' });

      // Cache should be invalidated after update
      prismaMock.trainingProfile.findUnique.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);
      await service.getById('test-id');

      expect(prismaMock.trainingProfile.findUnique).toHaveBeenCalledTimes(3);
    });
  });
  ...
  describe('error handling', () => {
    it('should handle network errors', async () => {
      prismaMock.trainingProfile.findMany.mockRejectedValue(new Error('ECONNREFUSED'));

      await expect(service.getAll()).rejects.toThrow('Database connection failed');
    });

    it('should handle timeout errors', async () => {
      prismaMock.trainingProfile.findMany.mockRejectedValue(new Error('Query timeout'));

      await expect(service.getAll()).rejects.toThrow('Query timeout');
    });

    it('should retry on transient errors', async () => {
      prismaMock.trainingProfile.findUnique
      ...
      expect(prismaMock.trainingProfile.findUnique).toHaveBeenCalledTimes(2);
    });
  });
});

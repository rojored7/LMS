 ...
 * Tests exhaustivos para el servicio de analytics
 */
...
describe('AnalyticsService', () => {
  ...
  describe('getAll', () => {
    it('should return all items', async () => {
      ...
      prismaMock.analytics.findMany.mockResolvedValue(mockData as any);

      const result = await service.getAll();

      expect(result).toEqual(mockData);
      expect(prismaMock.analytics.findMany).toHaveBeenCalledTimes(1);
    });
    ...
    it('should filter by query parameters', async () => {
      ...
      expect(prismaMock.analytics.findMany).toHaveBeenCalledWith(
    ...
    it('should handle database errors', async () => {
      prismaMock.analytics.findMany.mockRejectedValue(new Error('Database error'));
  ...
  describe('getById', () => {
    it('should return item by id', async () => {
      ...
      prismaMock.analytics.findUnique.mockResolvedValue(mockItem as any);

      const result = await service.getById('test-id');

      expect(result).toEqual(mockItem);
      expect(prismaMock.analytics.findUnique).toHaveBeenCalledWith({
        where: { id: 'test-id' }
      });
    });

    it('should throw error if item not found', async () => {
      prismaMock.analytics.findUnique.mockResolvedValue(null);
  ...
  describe('create', () => {
    it('should create new item', async () => {
      ...
      prismaMock.analytics.create.mockResolvedValue(created as any);
    ...
    it('should handle unique constraint violations', async () => {
      ...
      prismaMock.analytics.create.mockRejectedValue({
        code: 'P2002',
        meta: { target: ['name'] }
      });
  ...
  describe('update', () => {
    it('should update existing item', async () => {
      ...
      prismaMock.analytics.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.analytics.update.mockResolvedValue(updated as any);

      const result = await service.update('test-id', updateData);

      expect(result).toEqual(updated);
      expect(prismaMock.analytics.update).toHaveBeenCalledWith({
        where: { id: 'test-id' },
        data: updateData
      });
    });
    ...
    it('should handle partial updates', async () => {
      ...
      prismaMock.analytics.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.analytics.update.mockResolvedValue({ id: 'test-id', ...partialUpdate } as any);
  ...
  describe('delete', () => {
    it('should delete item', async () => {
      prismaMock.analytics.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.analytics.delete.mockResolvedValue({ id: 'test-id' } as any);

      await service.delete('test-id');

      expect(prismaMock.analytics.delete).toHaveBeenCalledWith({
        where: { id: 'test-id' }
      });
    });
    ...
    it('should handle deletion with dependencies', async () => {
      prismaMock.analytics.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.analytics.delete.mockRejectedValue({
        code: 'P2003',
    ...
    it('should soft delete when configured', async () => {
      prismaMock.analytics.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.analytics.update.mockResolvedValue({
        id: 'test-id',
        deletedAt: new Date()
      } as any);
  ...
  describe('caching', () => {
    it('should cache frequently accessed items', async () => {
      ...
      expect(prismaMock.analytics.findUnique).toHaveBeenCalledTimes(1);
    });
    ...
    it('should invalidate cache on update', async () => {
      ...
      prismaMock.analytics.findUnique.mockResolvedValue(mockItem as any);
      prismaMock.analytics.update.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);

      await service.getById('test-id');
      await service.update('test-id', { name: 'Updated' });

      // Cache should be invalidated after update
      prismaMock.analytics.findUnique.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);
      await service.getById('test-id');

      expect(prismaMock.analytics.findUnique).toHaveBeenCalledTimes(3);
    });
  });
  ...
  describe('error handling', () => {
    it('should handle network errors', async () => {
      prismaMock.analytics.findMany.mockRejectedValue(new Error('ECONNREFUSED'));

      await expect(service.getAll()).rejects.toThrow('Database connection failed');
    });

    it('should handle timeout errors', async () => {
      prismaMock.analytics.findMany.mockRejectedValue(new Error('Query timeout'));

      await expect(service.getAll()).rejects.toThrow('Query timeout');
    });

    it('should retry on transient errors', async () => {
      prismaMock.analytics.findUnique
      ...
      expect(prismaMock.analytics.findUnique).toHaveBeenCalledTimes(2);
    });
  });
});

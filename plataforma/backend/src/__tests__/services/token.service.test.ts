/**
 * token Service Tests
 * Tests exhaustivos para el servicio de token
 */
...
describe('TokenService', () => {
  ...
  describe('getAll', () => {
    it('should return all items', async () => {
      ...
      prismaMock.token.findMany.mockResolvedValue(mockData as any);
    ...
    it('should handle database errors', async () => {
      prismaMock.token.findMany.mockRejectedValue(new Error('Database error'));
  ...
  describe('getById', () => {
    it('should return item by id', async () => {
      ...
      prismaMock.token.findUnique.mockResolvedValue(mockItem as any);
  ...
  describe('create', () => {
    it('should create new item', async () => {
      ...
      prismaMock.token.create.mockResolvedValue(created as any);
    ...
    it('should handle unique constraint violations', async () => {
      ...
      prismaMock.token.create.mockRejectedValue({
        code: 'P2002',
        meta: { target: ['name'] }
      });
  ...
  describe('update', () => {
    it('should update existing item', async () => {
      ...
      prismaMock.token.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.token.update.mockResolvedValue(updated as any);
    ...
    it('should handle partial updates', async () => {
      ...
      prismaMock.token.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.token.update.mockResolvedValue({ id: 'test-id', ...partialUpdate } as any);
  ...
  describe('delete', () => {
    it('should delete item', async () => {
      prismaMock.token.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.token.delete.mockResolvedValue({ id: 'test-id' } as any);
    ...
    it('should handle deletion with dependencies', async () => {
      prismaMock.token.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.token.delete.mockRejectedValue({
        code: 'P2003',
  ...
  describe('caching', () => {
    ...
    it('should invalidate cache on update', async () => {
      ...
      prismaMock.token.findUnique.mockResolvedValue(mockItem as any);
      prismaMock.token.update.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);

      await service.getById('test-id');
      await service.update('test-id', { name: 'Updated' });

      // Cache should be invalidated after update
      prismaMock.token.findUnique.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);
      await service.getById('test-id');

      expect(prismaMock.token.findUnique).toHaveBeenCalledTimes(3);
    });
  });
  ...
  describe('error handling', () => {
    it('should handle network errors', async () => {
      prismaMock.token.findMany.mockRejectedValue(new Error('ECONNREFUSED'));

      await expect(service.getAll()).rejects.toThrow('Database connection failed');
    });

    it('should handle timeout errors', async () => {
      prismaMock.token.findMany.mockRejectedValue(new Error('Query timeout'));

      await expect(service.getAll()).rejects.toThrow('Query timeout');
    });

    it('should retry on transient errors', async () => {
      prismaMock.token.findUnique

/**
 * admin Service Tests
 * Tests exhaustivos para el servicio de admin
 */
...
describe('AdminService', () => {
  ...
  describe('getAll', () => {
    it('should return all items', async () => {
      ...
      prismaMock.admin.findMany.mockResolvedValue(mockData as any);
    ...
    it('should handle database errors', async () => {
      prismaMock.admin.findMany.mockRejectedValue(new Error('Database error'));
  ...
  describe('getById', () => {
    it('should return item by id', async () => {
      ...
      prismaMock.admin.findUnique.mockResolvedValue(mockItem as any);
  ...
  describe('create', () => {
    it('should create new item', async () => {
      ...
      prismaMock.admin.create.mockResolvedValue(created as any);
    ...
    it('should handle unique constraint violations', async () => {
      ...
      prismaMock.admin.create.mockRejectedValue({
        code: 'P2002',
        meta: { target: ['name'] }
      });
  ...
  describe('update', () => {
    it('should update existing item', async () => {
      ...
      prismaMock.admin.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.admin.update.mockResolvedValue(updated as any);
    ...
    it('should handle partial updates', async () => {
      ...
      prismaMock.admin.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.admin.update.mockResolvedValue({ id: 'test-id', ...partialUpdate } as any);
  ...
  describe('delete', () => {
    it('should delete item', async () => {
      prismaMock.admin.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.admin.delete.mockResolvedValue({ id: 'test-id' } as any);
    ...
    it('should handle deletion with dependencies', async () => {
      prismaMock.admin.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.admin.delete.mockRejectedValue({
        code: 'P2003',
  ...
  describe('caching', () => {
    it('should cache frequently accessed items', async () => {
      ...
      expect(prismaMock.admin.findUnique).toHaveBeenCalledTimes(1);
    });
    ...
    it('should invalidate cache on update', async () => {
      ...
      prismaMock.admin.findUnique.mockResolvedValue(mockItem as any);
      prismaMock.admin.update.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);

      await service.getById('test-id');
      await service.update('test-id', { name: 'Updated' });

      // Cache should be invalidated after update
      prismaMock.admin.findUnique.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);
      await service.getById('test-id');

      expect(prismaMock.admin.findUnique).toHaveBeenCalledTimes(3);
    });
  });
  ...
  describe('error handling', () => {
    it('should handle network errors', async () => {
      prismaMock.admin.findMany.mockRejectedValue(new Error('ECONNREFUSED'));

      await expect(service.getAll()).rejects.toThrow('Database connection failed');
    });

    it('should handle timeout errors', async () => {
      prismaMock.admin.findMany.mockRejectedValue(new Error('Query timeout'));
    ...
    it('should retry on transient errors', async () => {
      ...
      expect(prismaMock.admin.findUnique).toHaveBeenCalledTimes(2);
    });
  });
});

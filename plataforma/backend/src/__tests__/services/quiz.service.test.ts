/**
 * quiz Service Tests
 * Tests exhaustivos para el servicio de quiz
 */
...
describe('QuizService', () => {
  ...
  describe('getAll', () => {
    it('should return all items', async () => {
      ...
      prismaMock.quiz.findMany.mockResolvedValue(mockData as any);
    ...
    it('should handle database errors', async () => {
      prismaMock.quiz.findMany.mockRejectedValue(new Error('Database error'));
  ...
  describe('getById', () => {
    it('should return item by id', async () => {
      ...
      prismaMock.quiz.findUnique.mockResolvedValue(mockItem as any);
  ...
  describe('create', () => {
    it('should create new item', async () => {
      ...
      prismaMock.quiz.create.mockResolvedValue(created as any);
  ...
  describe('update', () => {
    it('should update existing item', async () => {
      ...
      prismaMock.quiz.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.quiz.update.mockResolvedValue(updated as any);
    ...
    it('should handle partial updates', async () => {
      ...
      prismaMock.quiz.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.quiz.update.mockResolvedValue({ id: 'test-id', ...partialUpdate } as any);
  ...
  describe('delete', () => {
    it('should delete item', async () => {
      prismaMock.quiz.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.quiz.delete.mockResolvedValue({ id: 'test-id' } as any);
    ...
    it('should handle deletion with dependencies', async () => {
      prismaMock.quiz.findUnique.mockResolvedValue({ id: 'test-id' } as any);
      prismaMock.quiz.delete.mockRejectedValue({
        code: 'P2003',
  ...
  describe('caching', () => {
    ...
    it('should invalidate cache on update', async () => {
      ...
      prismaMock.quiz.findUnique.mockResolvedValue(mockItem as any);
      prismaMock.quiz.update.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);

      await service.getById('test-id');
      await service.update('test-id', { name: 'Updated' });

      // Cache should be invalidated after update
      prismaMock.quiz.findUnique.mockResolvedValue({ ...mockItem, name: 'Updated' } as any);
      await service.getById('test-id');

      expect(prismaMock.quiz.findUnique).toHaveBeenCalledTimes(3);
    });
  });
  ...
  describe('error handling', () => {
    it('should handle network errors', async () => {
      prismaMock.quiz.findMany.mockRejectedValue(new Error('ECONNREFUSED'));

      await expect(service.getAll()).rejects.toThrow('Database connection failed');
    });

    it('should handle timeout errors', async () => {
      prismaMock.quiz.findMany.mockRejectedValue(new Error('Query timeout'));
    ...
    it('should retry on transient errors', async () => {
      ...
      expect(prismaMock.quiz.findUnique).toHaveBeenCalledTimes(2);
    });
  });
});
